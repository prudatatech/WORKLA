-- Migration: 117_repair_accept_engine.sql
-- Fixes: "Could not find the function public.accept_job_offer_rpc in the schema cache"
-- Purpose: Restores the atomic job acceptance engine.
...
BEGIN
  ...
  -- 2. Atomic Update: Assign Provider and confirm booking
  UPDATE bookings
  SET 
  ...
  RETURNING status, customer_id INTO v_booking_status, v_customer_id;

  -- If no row was updated, someone else took it or it was cancelled.
  IF NOT FOUND THEN
...
SELECT 'Repair: accept_job_offer_rpc engine restored successfully ✅' AS result;
