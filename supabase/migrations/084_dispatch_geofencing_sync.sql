-- ==============================================================
-- GEO-FENCING DISPATCH SYNC
-- Purpose: Link bookings to service zones and update dispatch engine.
-- ==============================================================

-- 1. Add service_zone_id to bookings
ALTER TABLE public.bookings
    ADD COLUMN IF NOT EXISTS service_zone_id UUID REFERENCES public.service_zones(id);

-- 2. Update existing bookings (best effort)
UPDATE public.bookings b
SET service_zone_id = (
    SELECT id FROM public.service_zones 
    WHERE status = 'active' 
      AND ST_Contains(boundary, ST_SetSRID(ST_Point(b.customer_longitude, b.customer_latitude), 4326))
    LIMIT 1
)
WHERE service_zone_id IS NULL;

-- 3. Update dispatch_job function to be zone-aware
    ...
    FOR v_provider IN
        SELECT * FROM (
            ...
            WHERE pd.is_online = TRUE
              ...
              -- OPTIONAL: Add zone-based provider filtering if providers are assigned to zones
        ) sub
...
SELECT 'Dispatch Geo-fencing Sync Complete ✅' AS result;
