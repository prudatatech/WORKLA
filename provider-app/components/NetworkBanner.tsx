    ...
    useEffect(() => {
        const unsubscribe = NetInfo.addEventListener(state => {
            ...
            if (state.isConnected === false) {
                ...
                // We're back online!
                if (!isRecovering && visible) hideBanner();
                // Process any offline actions that were queued
                processSyncQueue();
            }
        });

        return () => unsubscribe();
    }, [visible, isRecovering, showBanner, hideBanner]);
...
const styles = StyleSheet.create({
    ...
    offline: {
        backgroundColor: '#D97706', // Amber warning for provider offline
        borderColor: '#B45309',
    },
    recovering: {
